#!/usr/bin/env python3
"""Build a reproducible pairwise or four-variant MAHILDA comparison."""
...
try:
    from scripts.capture_mahilda_baseline import collect_sqlite_metadata, normalize_rule
except ModuleNotFoundError:  # Support ``python scripts/compare_mahilda_runs.py``.
    from capture_mahilda_baseline import collect_sqlite_metadata, normalize_rule
...
def _run_record(root: Path, database: str, source_dir: Path, expected_revision: str | None = None) -> dict[str, Any]:
    run_dir = root / "results" / "MAHILDA" / f"MAHILDA_{database}"
    progress_path = root / "results" / "progress" / f"MAHILDA_{database}.db.json"
    record: dict[str, Any] = {
        "database": database,
        "status": "incomplete",
        "failure": None,
    ...
    result = run_dir / f"MAHILDA_{database}_results.json"
...
def write_comparison(archive: Path, comparison: dict[str, Any]) -> None:
    ...
    lines = [
        "# Laptop MAHILDA four-variant comparison",
        "",
