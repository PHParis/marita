#!/usr/bin/env python3
"""Capture and summarize a read-only MAHILDA benchmark run.
...
def parse_log_tree(log_root: Path) -> list[dict[str, Any]]:
    """Parse every ``global.log`` below a captured MAHILDA log tree."""
...
def parse_constraint_graph(value: Any) -> dict[str, Any]:
    """Extract graph totals from the string serialized by MAHILDA."""
...
def parse_artifact_snapshot(results_root: Path) -> tuple[dict[str, dict[str, Any]], list[dict[str, Any]]]:
    ...
    for path in sorted(results_root.glob("MAHILDA_*/**/*.json")):
        parent = path.parent.name
        if not parent.startswith("MAHILDA_"):
            continue
        database = parent.removeprefix("MAHILDA_")
    ...
    for path in sorted(results_root.glob("report_MAHILDA_*.md")):
        database = path.stem.removeprefix("report_MAHILDA_")
        snapshot = snapshot_for(database)
    ...
    *,
    ...
    return {
        ...
        "capture": {
            ...
            "scope": "MAHILDA/MARITA evidence only; source databases are referenced by hash and not copied",
        },
        "source": source_paths,
        ...
        "artifacts": {
            ...
            "raw_results_root": "raw/results/MAHILDA",
            "raw_logs_root": "raw/logs/MAHILDA",
        },
        "databases": databases,
...
def capture(args: argparse.Namespace) -> dict[str, Any]:
    ...
    specs = [
        CopySpec(args.source_results, archive_root / "raw" / "results" / "MAHILDA"),
        CopySpec(args.source_logs, archive_root / "raw" / "logs" / "MAHILDA"),
        CopySpec(args.source_timings, archive_root / "raw" / "logs" / "timings", ("mahilda_*.stdout",)),
    ]
    ...
    attempts = parse_log_tree(archive_root / "raw" / "logs" / "MAHILDA")
    results_root = archive_root / "raw" / "results" / "MAHILDA"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--source-results", required=True, help="Remote MAHILDA result directory.")
    parser.add_argument("--source-logs", required=True, help="Remote MAHILDA global-log directory.")
    ...
    parser.add_argument("--output", default=f"results/mahilda_tipi01_{today}")
    parser.add_argument("--manifest", default=f"research/generated/mahilda_baseline_{today}.json")
    return parser
