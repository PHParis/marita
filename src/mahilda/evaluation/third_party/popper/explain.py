...
class Explainer:
    ...
    def build_test_prog(self, subprog, directions):
        ...
        for head, body in subprog:
            if head:
                head_modes = tuple(
                    directions[head.predicate][i] for i in range(head.arity)
                )
            ...
            for body_literal in body:
                body_modes = tuple(
                    directions[body_literal.predicate][i]
                    for i in range(body_literal.arity)
                )
                body_literals.add(
                    Literal(
                        body_literal.predicate,
                        body_literal.arguments,
                        body_modes,
                    )
                )
    ...
    def explain_totally_incomplete_aux(
        self, prog, directions, depth, sat=set(), unsat=set()
    ):
        ...
        for subprog in find_subprogs(prog, has_recursion):
            ...
            if (
                len(subprog) > 1
                and has_recursion
                and any(not recursive_input_is_ok(rule) for rule in subprog)
            ):
                continue
            ...
            unsat.add(raw_prog)
            xs = list(
                self.explain_totally_incomplete_aux(
                    subprog, directions, depth + 1, sat, unsat
                )
            )
            if len(xs):
                yield from xs
...
def find_subrules_aux(rule, force_head, recursive):
    ...
    if recursive and rule_is_recursive(rule):
        # recursive literals
        b1 = frozenset(
            literal for literal in body if literal.predicate == head.predicate
        )
