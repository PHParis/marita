...
class Tester:
    ...
    def __init__(self, settings):
        ...
        with self.settings.stats.duration("load data"):
            for x in [exs_pl_path, bk_pl_path, test_pl_path]:
                if (
                    os.name == "nt"
                ):  # if on Windows, SWI requires escaped directory separators
                    x = x.replace("\\", "\\\\")
                self.prolog.consult(x)

        # load examples
        self.bool_query(f"load_examples")
        # FIX: this is a hack to get the number of positive and negative examples
        try:
    ...
    def has_redundant_rule_(self, prog):
        ...
        for head, body in prog:
            ...
            prog_.append(c)
        prog_ = f"[{','.join(prog_)}]"
        return len(list(self.prolog.query(f"redundant_clause({prog_})")))
