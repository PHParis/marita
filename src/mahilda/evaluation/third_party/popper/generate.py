...
def parse_model(model):
    ...
    for r1_index, lower_rule_indices in rule_index_ordering.items():
        ...
        rule_ordering[r1] = set(
            rule_lookup[r2_index] for r2_index in lower_rule_indices
        )
...
def build_rule_literals(rule, rule_var):
    ...
    yield Literal(
        ...
        (
            ...
            head.predicate,
            len(head.arguments),
            tuple(vo_variable2(rule_var, v) for v in head.arguments),
        ),
    )
    ...
    for body_literal in body:
        yield Literal(
            ...
            (
                rule_var,
                body_literal.predicate,
                len(body_literal.arguments),
                tuple(vo_variable2(rule_var, v) for v in body_literal.arguments),
            ),
        )
...
class Generator:
    ...
    def get_ground_rules(self, rule):
        ...
        assignments = self.grounder.find_bindings(
            rule, self.settings.max_rules, self.settings.max_vars
        )
    ...
    def get_ground_deep_rules(self, body):
        # find bindings for variables in the rule
        assignments = self.grounder.find_deep_bindings(
            body, self.settings.max_rules, self.settings.max_vars
        )
        # ground the rule for each variable assignment
    ...
    def redundancy_constraint1(self, prog, rule_ordering={}):
        ...
        literals.append(
            Literal(
                "recursive_clause",
                (rule_var, head.predicate, len(head.arguments)),
            )
        )
    ...
    def redundancy_constraint4(self, program, rule_ordering={}):
        ...
        r1 = (
            Literal("not_redundant", (clause_variable, prog_handle)),
            (
                Literal(
                    "specialises",
                    (clause_variable, prog_handle),
                    positive=False,
                ),
            ),
        )
        r2 = (
            Literal("not_redundant", (clause_var1, prog_handle)),
            (
            ...
            for rule in prog:
                ...
                for body_literal in body:
                    if body_literal.predicate not in lits_num_rules:
                        continue
                    if (body_literal.predicate != head.predicate and is_rec) or (
                        head.predicate in recursively_called
                    ):
                        something_added |= (
                            not body_literal.predicate in recursively_called
                        )
...
class Grounder:
    ...
    def find_bindings(self, rule, max_rules, max_vars):
        ...
        rule_var_to_int = {
            v: i
            for i, v in enumerate(var for var in all_vars if isinstance(var, RuleVar))
        }
        ...
        for lit in body:
            ...
            if lit.predicate == "==":
                ...
                encoding.append(
                    f":- not bind_var({rule_var_int},{var_var_int},{value})."
                )
            elif lit.predicate == ">=":
                ...
                if isinstance(b, int):
                    ...
                    encoding.append(
                        f":- bind_rule({rule_var_int1},Val1), bind_rule({rule_var_int2},Val2), Val1>=Val2."
                    )
