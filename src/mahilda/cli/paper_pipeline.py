...
def _b3_jobs(
    ...
    *,
    ...
    jobs: list[dict[str, Any]] = []
    for database in _stage_order(_selected_databases(stage, databases), stage.id, "MAHILDA"):
...
def _mahilda_config(
    *,
    ...
    return {
        ...
        "algorithm": {"name": "MAHILDA", "parameters": merged_parameters},
        "mlflow": {"use": False},
    }
...
def write_b3_comparison(database_dir: Path) -> None:
    ...
    for db_path in sorted(database_dir.glob("*.db"), key=lambda path: path.name.lower()):
        db = db_path.stem
        fk_rules = Path(f"results/ablation_join/fk_only/{db}/MAHILDA_{db}/MAHILDA_{db}_results.json")
        fj_rules = Path(f"results/ablation_join/full_join/{db}/MAHILDA_{db}/MAHILDA_{db}_results.json")
        fk_et = Path(f"results/ablation_join/fk_only/{db}/MAHILDA_{db}/execution_time_{db}.json")
        fj_et = Path(f"results/ablation_join/full_join/{db}/MAHILDA_{db}/execution_time_{db}.json")
        if not fk_et.exists():
