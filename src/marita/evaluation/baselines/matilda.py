...
from marita.utils.rules import Predicate, Rule, TGDRule
...
class Matilda(BaseAlgorithm):
    ...
    def _resolve_matilda_path(configured_path: Any) -> Path:
        raw_path = configured_path or os.environ.get("MARITA_MATILDA_PATH")
        if raw_path:
        ...
        if not src_path.is_dir():
            raise RuntimeError(
                ...
                f"MARITA_MATILDA_PATH to a MATILDA repo containing src/: {path}"
            )
        return path
