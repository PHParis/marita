...
class RelationalDownloadSettings:
    ...
    def from_env(cls, output_dir: Path, *, timeout: int = 300) -> RelationalDownloadSettings:
        return cls(
            ...
            base_url=os.getenv("MARITA_RELATIONAL_BASE_URL", DEFAULT_BASE_URL).rstrip("/"),
            host=os.getenv("MARITA_RELATIONAL_HOST", DEFAULT_HOST),
            port=int(os.getenv("MARITA_RELATIONAL_PORT", str(DEFAULT_PORT))),
            user=os.getenv("MARITA_RELATIONAL_USER", DEFAULT_USER),
            password=os.getenv("MARITA_RELATIONAL_PASSWORD", DEFAULT_PASSWORD),
            dump_command=os.getenv("MARITA_RELATIONAL_DUMP_COMMAND", "mysqldump"),
            timeout=timeout,
        )
