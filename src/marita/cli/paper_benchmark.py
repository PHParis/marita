...
from marita.cli.artifacts import format_duration
...
class EmailNotificationConfig:
    ...
    smtp_password_env: str = "MARITA_SMTP_PASSWORD"
    starttls: bool = False
...
def parse_arguments(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--algorithms",
        default=None,
        help="Comma-separated algorithms to run: MARITA, AMIE3, SPIDER, POPPER, MATILDA, or ALL (default: MARITA).",
    )
    parser.add_argument(
        "--databases",
        default=None,
    ...
    parser.add_argument(
        "--smtp-password-env",
        default="MARITA_SMTP_PASSWORD",
        help="Environment variable containing the SMTP password (default: MARITA_SMTP_PASSWORD).",
    )
...
def _email_config_from_args(args: argparse.Namespace) -> EmailNotificationConfig | None:
    recipient = args.email_to or os.environ.get("MARITA_EMAIL_TO")
    if not recipient:
        return None

    smtp_host = args.smtp_host or os.environ.get("MARITA_SMTP_HOST") or "localhost"
    smtp_port = args.smtp_port or int(os.environ.get("MARITA_SMTP_PORT", "25"))
    sender = args.email_from or os.environ.get("MARITA_EMAIL_FROM") or recipient
    smtp_user = args.smtp_user or os.environ.get("MARITA_SMTP_USER")
    starttls = args.smtp_starttls or _env_flag("MARITA_SMTP_STARTTLS")
...
def _send_email_notification(
    ...
    *,
    ...
    message["Subject"] = f"MARITA paper-benchmark {status} on {host_name}"
    message.set_content(
        _format_email_body(
            status=status,
            exit_code=exit_code,
...
def _format_email_body(
    *,
    ...
    lines = [
        "MARITA paper benchmark notification",
        "",
        f"Status: {status}",
...
def _profile_workers(profile: dict[str, Any]) -> dict[str, int]:
    ...
    defaults = {"MARITA": 1, "SPIDER": 1, "AMIE3": 1, "POPPER": 1, "MATILDA": 1}
