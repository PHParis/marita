...
from marita.cli.artifacts import build_command_artifacts, write_execution_time_metrics, write_markdown_report
from marita.database.alchemy_utility import AlchemyUtility
from marita.evaluation.baselines import Amie3, Matilda, Popper, Spider
...
class DatabaseProcessor:
    """Orchestrates single-database MARITA rule discovery."""
    ...
    def __init__(
        ...
        if normalized_algorithm != "MARITA":
            msg = (
                "`marita run` only supports the MARITA algorithm. "
                "Use `marita benchmark --baseline ...` for competitor baselines."
            )
    ...
    def discover_rules(self) -> int:
        ...
        verbose = os.environ.get("MARITA_VERBOSE") == "1"
        quiet = os.environ.get("MARITA_QUIET") == "1"

        selected_algorithm = MARITA
