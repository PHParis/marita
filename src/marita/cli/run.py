...
def main(argv: list[str] | None = None) -> int:
    ...
    if algorithm_name != "MARITA":
        logger.error(
            "`marita run` only supports MARITA. Use `marita benchmark --baseline %s` for baselines.",
            algorithm_name,
        )
        return 1
    ...
    try:
        with scoped_env_vars({"MARITA_LOG_DIR": str(log_dir)}), mlflow_run_context(use_mlflow, config.raw):
            if use_mlflow and not quiet:
