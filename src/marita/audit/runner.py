...
from marita.audit.amie_translation import translate_amie_source, validate_mapping_artifacts
...
from marita.audit.evaluator import AuditEvaluationError, SQLiteRuleEvaluator
from marita.audit.matching import covered_on_instance, subsumes
...
from marita.audit.parsing import load_source_rules, parse_formula, parse_popper_source, parse_source_rule
...
def _write_paper_table_tex(path: Path, records: list[AuditRecord], config: AuditConfig) -> None:
    rows = _exclusions_by_algorithm(records, config.competitors)
    lines = [
        "% Generated by `marita audit`; do not edit manually.",
...
def _write_diagnosis_markdown(
    ...
    *,
    ...
    lines = [
        "# Audit Diagnosis",
        "",
        "This report explains why rules are or are not usable in the MARITA coverage claim.",
        "",
        "## Scope Status Counts",
        "",
    ]
...
def _write_claims_markdown(path: Path, records: list[AuditRecord], config: AuditConfig) -> None:
    ...
    if not partitions_valid:
        ...
        lines.append(
            f"- After the reported exclusions, MARITA recovers all {comparable} unique comparable exact "
            "competitor rules under alpha-equivalence (100% empirical recall on the audited outputs)."
        )
    else:
        lines.append("- MARITA does not achieve 100% alpha-equivalence recall on the audited outputs.")
    ...
    lines.extend(
        [
            ...
            "## Not Supported",
            "",
            "- The audit does not prove that MARITA finds all true rules in the database.",
