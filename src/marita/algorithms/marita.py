...
import marita.algorithms.marita_core.tgd_discovery as marita_core
from marita.algorithms.base_algorithm import BaseAlgorithm
from marita.algorithms.marita_core.tgd_discovery import dfs, init, instantiate_tgd, path_pruning
from marita.utils.rules import MARITARule, Predicate, TGDRule
...
class HornRuleExtended:
    """Compatibility data structure for legacy consumers of MARITA."""
...
class MARITA(BaseAlgorithm):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        logger.debug("MARITA initialised with settings: %s", self.settings)
    ...
    def _apply_overrides(base: Dict[str, Any], overrides: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for raw_key, value in overrides.items():
            ...
            normalised_key = MARITA.KEY_NORMALISATION.get(raw_key, raw_key)
            ...
            if normalised_key == "disjoint_semantics":
                merged["disjoint_semantics"] = MARITA._to_bool(value)
                continue

            if normalised_key == "joinability":
                merged["joinability"] = MARITA._coerce_joinability(value)
                continue
    ...
    def discover_rules(self, **kwargs: Any) -> Generator[MARITARule, None, None]:  # type: ignore[override]
        ...
        joinability = MARITA._coerce_joinability(runtime_settings.get("joinability", "fk"))
        ...
        if not results_path:
            temp_results_dir = tempfile.mkdtemp(prefix="marita_results_")
            results_path = temp_results_dir
        os.makedirs(str(results_path), exist_ok=True)
        results_path_str = str(results_path)

        previous_disjoint = marita_core.APPLY_DISJOINT
        previous_threshold = marita_core.SUPPORT_THRESHOLD
        previous_full_join = marita_core.APPLY_FULL_JOINABILITY
        marita_core.APPLY_DISJOINT = disjoint_semantics
        marita_core.SUPPORT_THRESHOLD = support_threshold
        marita_core.APPLY_FULL_JOINABILITY = joinability == "full"
        ...
        try:
            ...
            marita_core.APPLY_DISJOINT = previous_disjoint
            marita_core.SUPPORT_THRESHOLD = previous_threshold
            marita_core.APPLY_FULL_JOINABILITY = previous_full_join
