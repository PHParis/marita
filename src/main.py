...
from algorithms.mahilda import MAHILDA
...
class DatabaseProcessor:
            ...
            with AlchemyUtility(
                ...
                if not quiet:
                    ...
                    if self.algorithm_name.upper() == "MAHILDA":
                        self.logger.info(f"MAHILDA settings: nb_occurrence=3, max_table=3, max_vars=6")
                
                for rule in algo.discover_rules(results_dir=str(unique_results_dir)):
                    rule_count += 1
                    rules.append(rule)
                    
                    # Beautiful rule display
                    if verbose:
                        accuracy_color = Fore.GREEN if rule.accuracy >= 0.8 else Fore.YELLOW if rule.accuracy >= 0.5 else Fore.RED
                        conf_color = Fore.GREEN if rule.confidence >= 0.8 else Fore.YELLOW if rule.confidence >= 0.5 else Fore.RED
                ...
                if not quiet:
                    self.logger.info(
                        f"{Fore.GREEN}{Style.BRIGHT}Discovered {number_of_rules} rules.{Style.RESET_ALL}"
                    )
    ...
    def generate_report(self, number_of_rules: int, result_path: Path, top_rules: List[dict], execution_time: float = None) -> None:
...
The rule discovery process was completed successfully. The discovered rules have been saved to the specified results path.
    ...
    """
        ...
        if execution_time is not None:
            import json
            time_metrics_file = self.results_dir / f"MAHILDA_{self.database_name.stem}" / f"execution_time_{self.database_name.stem}.json"
            time_metrics = {
