...
The following commands run MARITA on the  [example configuration file](https://github.com/PHParis/marita/blob/main/configs/config.example.yaml), which uses the TRAINS database. 

To run MARITA on your own database, make a copy of the [example configuration file](https://github.com/PHParis/marita/blob/main/configs/config.example.yaml), and then run MARITA with `--config your_file`.

### Linux
```bash
uv sync
uv run marita    # runs on the example config file
```
...
python -m marita                                                          # runs on the example config file
```
