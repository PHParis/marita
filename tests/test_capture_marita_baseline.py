...
from scripts.capture_marita_baseline import (
...
"""
    ...
    attempts = parse_log_attempts(text, "MARITA/demo/global.log")
...
def test_artifact_snapshot_handles_empty_malformed_and_metric_only_files(tmp_path: Path) -> None:
    ...
    complete = results / "MARITA_demo"
    complete.mkdir(parents=True)
    (complete / "MARITA_demo_results.json").write_text("[]", encoding="utf-8")
    ...
    (results / "report_MARITA_demo.md").write_text("**Number of Rules Discovered:** 0", encoding="utf-8")

    partial = results / "MARITA_partial"
    ...
    malformed = results / "MARITA_bad"
    malformed.mkdir(parents=True)
    (malformed / "MARITA_bad_results.json").write_text("{malformed", encoding="utf-8")
