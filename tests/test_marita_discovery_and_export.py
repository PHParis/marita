...
from marita.algorithms import marita as marita_module
from marita.algorithms.marita import MARITA, HornRuleExtended, RuleConversionError
from marita.algorithms.marita_core.constraint_graph import IndexedAttribute, JoinableIndexedAttributes
...
def test_discover_rules_handles_empty_jia_and_restores_globals(monkeypatch) -> None:
    algorithm = MARITA(database=object(), settings={"disjoint_semantics": True, "support_threshold": 1})
    old_disjoint = marita_module.marita_core.APPLY_DISJOINT
    old_threshold = marita_module.marita_core.SUPPORT_THRESHOLD
    old_full_join = marita_module.marita_core.APPLY_FULL_JOINABILITY
    old_full_join = marita_module.marita_core.APPLY_FULL_JOINABILITY
    ...
    monkeypatch.setattr(marita_module, "init", fake_init)

    discovered = list(algorithm.discover_rules())
    assert discovered == []
    assert old_disjoint == marita_module.marita_core.APPLY_DISJOINT
    assert old_threshold == marita_module.marita_core.SUPPORT_THRESHOLD
    assert old_full_join == marita_module.marita_core.APPLY_FULL_JOINABILITY


def test_discover_rules_yields_only_single_head_and_fails_on_conversion_errors(monkeypatch) -> None:
    algorithm = MARITA(database=object(), settings={"timeout": 100})
    ...
    monkeypatch.setattr(marita_module, "init", fake_init)
    monkeypatch.setattr(marita_module, "dfs", fake_dfs)
    monkeypatch.setattr(marita_module, "instantiate_tgd", lambda *args, **kwargs: "∀ x: A(x=x) ⇒ B(x=x)")
    monkeypatch.setattr(marita_module.TGDRuleFactory, "str_to_tgd", fake_str_to_tgd)
...
def test_discover_rules_emits_multi_column_body_without_conversion_loss(monkeypatch) -> None:
    algorithm = MARITA(database=object(), settings={"support_threshold": 1})
    ia1 = IndexedAttribute(0, 0, 0)
    ia2 = IndexedAttribute(1, 0, 0)
    jia = JoinableIndexedAttributes(ia1, ia2)

    monkeypatch.setattr(marita_module, "init", lambda *args, **kwargs: (object(), object(), [jia]))
    monkeypatch.setattr(
        marita_module,
        "dfs",
...
def test_discover_rules_respects_should_stop_and_timeout(monkeypatch) -> None:
    ...
    monkeypatch.setattr(marita_module, "init", lambda *args, **kwargs: (object(), object(), [jia]))
    monkeypatch.setattr(marita_module, "dfs", lambda *args, **kwargs: iter(()))
...
def test_horn_rule_extended_and_export_and_stats(tmp_path: Path, monkeypatch) -> None:
    ...
    stats = MARITA.get_horn_rule_statistics(rules)
    assert stats["horn_rules"] == 1
    assert stats["average_support"] == 0.2

    out = tmp_path / "rules.txt"
    MARITA.export_horn_rules(rules, str(out))
    assert out.exists()
    assert out.read_text(encoding="utf-8").strip() == "r1"


def test_coercion_helpers() -> None:
    assert MARITA._to_bool("yes") is True
    assert MARITA._to_bool("0") is False
    assert MARITA._coerce_positive_int("4", 2) == 4
    assert MARITA._coerce_positive_int("x", 2) == 2
    assert MARITA._coerce_timeout("10") == 10
    assert MARITA._coerce_timeout("0") is None
    assert MARITA._coerce_float("1.5", 0.0) == 1.5
    assert MARITA._coerce_float("bad", 0.0) == 0.0


def test_joinability_coercion() -> None:
    assert MARITA._coerce_joinability(True) == "full"
    assert MARITA._coerce_joinability(False) == "fk"
    assert MARITA._coerce_joinability(None) == "fk"
    assert MARITA._coerce_joinability("full") == "full"
    assert MARITA._coerce_joinability("fk") == "fk"
    assert MARITA._coerce_joinability("  FULL  ") == "full"
