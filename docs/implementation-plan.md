# MARITA Reorg Follow-Up Implementation Plan

## Recommendations

- Save this plan at `docs/implementation-plan.md` and keep a short link to it from `README.md`.
- Keep `src/marita/evaluation/third_party/**`, `legacy/**`, and `research/**` out of scope for refactoring unless a task explicitly says otherwise.
...
- Expand Ruff and Pyright coverage gradually, starting with CLI and utils, then `algorithms/marita.py`, while continuing to exclude vendored code.
...
- Preserve the current command split: `run` for `MARITA`, `benchmark` for baselines, `batch` for multi-database execution.
...
- [x] Replace machine-specific defaults in `src/marita/cli/main.py`, `src/marita/cli/batch.py`, `src/marita/cli/test_data.py`, `README.md`, and `configs/config.example.yaml`; remove `/Volumes/backup_mac_1/...` defaults and make all documented commands runnable from a fresh checkout.
- [x] Make `configs/config.example.yaml` point at repo-local data that can actually exist, ideally `../test_data/test.db` after `uv run marita test-db`, and update `README.md` quick start so the documented path matches reality.
- [x] Extract shared runtime helpers from `src/marita/cli/run.py` and `src/marita/cli/benchmark.py` into a common module for config loading, directory creation, MLflow setup, logger setup, cleanup, and report/artifact handling.
- [x] Refactor `src/marita/cli/batch.py` to use the same shared runtime path as `run` and `benchmark` instead of maintaining a standalone execution model.
- [x] Make `src/marita/utils/logging_utils.py` idempotent by avoiding repeated root-handler attachment and by returning named loggers instead of mutating the global logger on every call.
- [x] Standardize command output across `src/marita/cli/run.py`, `src/marita/cli/benchmark.py`, `src/marita/cli/batch.py`, `src/marita/cli/smoke.py`, `src/marita/cli/test_data.py`, `src/marita/cli/mlflow_start.py`, and `src/marita/cli/mlflow_ui.py`; remove mixed English/French output and reduce ad hoc `print()` usage.
- [x] Wire `src/marita/utils/monitor.py` into actual cancellation by passing a stop predicate into `MARITA.discover_rules()` from `src/marita/cli/run.py`; ensure the monitor is stopped and joined on success, failure, and signal exit.
- [x] Replace the fragile shell-string parsing in `src/marita/utils/run_cmd.py` with a safer subprocess wrapper that uses structured argv input, explicit redirection, timeout handling, and logger-based error reporting.
- [x] Update `src/marita/evaluation/baselines/amie3.py`, `src/marita/evaluation/baselines/spider.py`, and `src/marita/evaluation/baselines/popper.py` to use the safer subprocess wrapper without changing vendored third-party internals.
...
- [x] Normalize environment-variable handling for `MARITA_VERBOSE`, `MARITA_QUIET`, and `MARITA_LOG_DIR` so command chaining inside one Python process does not leak settings across invocations.
- [x] Keep `marita run` restricted to `MARITA` and `marita benchmark` restricted to baselines, but make the error messages and help text explicit and consistent.

## Phase 1 Exit Criteria

- [x] `uv run marita test-db` works from a fresh checkout without path edits.
- [x] `uv run marita smoke` works from a fresh checkout without path edits.
...
- [x] Add explicit config validation in `src/marita/utils/config_loader.py` for required keys, allowed algorithm and baseline names, path fields, worker counts, timeouts, and MLflow configuration shape.
...
- [x] Add tests for `MARITA` parameter normalization in `src/marita/algorithms/marita.py`, including legacy key aliases such as `max_table`, `max_vars`, `nb_occurrence`, and `recursivity`.
- [x] Add tests for baseline normalization in `src/marita/cli/benchmark.py`, including alias handling such as `ILP -> POPPER`.
...
- [x] Reduce Ruff exclusions in `pyproject.toml` by first bringing `src/marita/cli/batch.py`, `src/marita/utils/logging_utils.py`, `src/marita/utils/run_cmd.py`, `src/marita/utils/rules.py`, and `src/marita/algorithms/marita.py` under linting.
...
- [x] Remove duplicate abstraction layers by consolidating `src/marita/algorithms/base_algorithm.py` and `src/marita/algorithms/rule_discovery_algorithm.py` if they remain functionally identical after Phase 1 and Phase 2.
...
- [x] Add `docs/cli.md` describing `marita run`, `marita benchmark`, `marita batch`, `marita smoke`, `marita test-db`, and MLflow helper behavior.
...
- [x] Run `uv run marita test-db`.
- [x] Run `uv run marita smoke`.
- [x] Run `uv run marita run --config configs/config.example.yaml`.
- [x] Run `uv run marita benchmark --config configs/config.example.yaml --baseline AMIE3` only in environments with Java available.
- [x] Run `uv run marita benchmark --config configs/config.example.yaml --baseline SPIDER` only in environments with Java available.
- [ ] Run `uv run marita benchmark --config configs/config.example.yaml --baseline POPPER` only in environments with `run-popper` available.

## Explicit Non-Goals

- [x] Do not refactor or restyle vendored code in `src/marita/evaluation/third_party/**`.
