# MARITA baseline capture

The tracked manifest `research/generated/marita_baseline_20260719.json` records
the MARITA/MARITA evidence captured from `tipi01` on 2026-07-19. The raw
archive is intentionally repository-local and ignored:
`results/marita_tipi01_20260719/`.

## Capture command

The collector performs read-only SSH commands and uses rsync only to copy data
to the local archive. It never runs MARITA and never copies a source database.
For the original capture, the equivalent command was:

```bash
uv run python scripts/capture_marita_baseline.py \
  --host tipi01 \
  --source-databases /people/paris/marita/data/relational \
  --source-results /people/paris/marita/results/paper_table2/MARITA \
  --source-logs /people/paris/marita/logs/paper_table2/MARITA \
  --source-timings /people/paris/marita/results/paper_table2/timings \
  --source-run-configs /people/paris/marita/results/paper_table2/configs \
  --source-config /people/paris/marita/configs/paper/benchmark_83.yaml \
  --source-pipeline-log /people/paris/marita/logs/paper_pipeline/runner_tipi01.log \
  --source-repo /people/paris/marita \
  --output results/marita_tipi01_20260719 \
  --manifest research/generated/marita_baseline_20260719.json
```
...
```text
results/marita_tipi01_YYYYMMDD/
├── raw/results/MARITA/       # result JSON, metric JSON, and reports
├── raw/logs/MARITA/          # per-database global/query logs
├── raw/logs/timings/          # marita_*.stdout files
├── raw/logs/pipeline/         # optional host pipeline log
├── provenance/config/         # benchmark profile
├── provenance/configs/        # per-database MARITA configs
...
```bash
uv run pytest tests/test_capture_marita_baseline.py
```
