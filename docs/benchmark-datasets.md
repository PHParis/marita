...
Run each database with the normal MAHILDA command and a config whose `database.name` is the corresponding `.db` file. The captured benchmark artifacts are in [`results/mahilda_tipi01_20260719`](../results/mahilda_tipi01_20260719/). Future runs should compare both rule hashes and rule counts, as well as runtime, against that stored baseline. Small runtime variation is expected; a changed hash set or count warrants investigation.
