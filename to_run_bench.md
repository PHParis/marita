...
- Full MAHILDA batch on all available converted relational DBs, or a fixed 54-dataset subset.
...
Two config files created. Here are the exact commands for each experiment:
  ...
  Do not use mahilda batch for any of these experiments. It constructs DatabaseProcessor without passing the config dict, so algorithm.parameters (walk_length, disjoint_semantics, etc.)
  silently reverts to defaults (walk_length=4, disjoint_semantics=false). Use mahilda paper-benchmark or mahilda run instead.

  ---
  Experiment 1 — Full MAHILDA batch on all 83 available DBs
  ...
  Results: results/full_83/MAHILDA/<db_stem>/ + results/full_83/summary.json.
  ...
  paper-benchmark enforces the memory limit but does not write peak RSS to summary.json. Use dry-run to generate individual configs, then wrap each mahilda run with /usr/bin/time -v:
  ...
  for cfg in results/peak_rss/configs/mahilda_*.yaml; do
    db=$(basename "$cfg" .yaml | sed 's/^mahilda_//')
    { /usr/bin/time -v uv run mahilda run --config "$cfg"; } 2>&1 \
  ...
  # YAGO core
  { /usr/bin/time -v uv run mahilda run --config configs/config.yago-core.yaml; } 2>&1 \
    | grep "Maximum resident set size" \
  ...
  mlflow:
    use: false
  YAMEOF
    uv run mahilda run --config /tmp/ablation_disjoint_N${N}.yaml
  ...
  mlflow:
    use: false
  YAMEOF
    uv run mahilda run --config /tmp/ablation_nondisjoint_N${N}.yaml
  done
  ...
  uv run mahilda run --config configs/config.yago-ontology-lite.yaml
